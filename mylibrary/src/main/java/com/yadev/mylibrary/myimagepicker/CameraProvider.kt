...
class CameraProvider(activity: ImagePickerActivity) : BaseProvider(activity) {
    ...
    private fun isPermissionGranted(context: Context): Boolean {
        ...
        if (mAskCameraPermission && PermissionUtil.isPermissionGranted(
                context,
                REQUIRED_PERMISSIONS_EXTENDED
            )
        ) {
            ...
            return true
        } else if (!mAskCameraPermission && PermissionUtil.isPermissionGranted(
                context,
                REQUIRED_PERMISSIONS
            )
        ) {
